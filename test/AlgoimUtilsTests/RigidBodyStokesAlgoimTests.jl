module RigidBodyStokesAgFEMTests
...
function dynamic(n::Int,w::Float64,
  ...
  function update_buffer!(t,dt,v₋₂)
    ...
    if buffer[].t == t
      ...
      Ωˢ,dΩˢ,_ = TriangulationAndMeasure(Ω,squad)
      ...
      buffer[] = (Ωˡ=Ωˡ,Ωˢ=Ωˢ,dΩˡ=dΩˡ,dΩˢ=dΩˢ,
                  dΓ=dΓ,n_Γ=n_Γ,aggsˡ=aggsˡ,c₋=c₋,t=t)
      return true
  ...
  function update_all!(t::Real,dt::Real,disp)
    ...
    Vˡ = AgFEMSpace(Vˡstd,aggsˡ,Vˡser)
    Qˡ = AgFEMSpace(Qˡstd,aggsˡ)
    Vˢ = ConstantFESpace(model,field_type=VectorValue{2,Float64})
    K  = ConstantFESpace(model)
    ...
    Y = MultiFieldFESpace([Vˡ,Qˡ,K,Vˢ,K])
    X = MultiFieldFESpace([Uˡ,Pˡ,L,Uˢ,L])
  ...
  σ(ε,ν) = 2*ν*ε
  τ(ε) = one(ε)
  meanᵗ(uˡ,pˡ,νˡ) = wˡ*(σ(ε(uˡ),νˡ))-pˡ*(τ∘(ε(uˡ)))

  a((uˡ,pˡ,lˡ,uˢ,lˢ),(vˡ,qˡ,kˡ,vˢ,kˢ)) =
    ∫( 2*νˡ*(ε(uˡ)⊙ε(vˡ)) - qˡ*(∇⋅uˡ) - (∇⋅vˡ)*pˡ )dΩˡ +
    ∫( pˡ*kˡ )dΩˡ + ∫( qˡ*lˡ )dΩˡ +
    ∫( (γ*νᵞ/h)*(jumpᵘ(uˡ,uˢ)⋅jumpᵘ(vˡ,vˢ)) - 
       (jumpᵘ(vˡ,vˢ)⋅(n_Γ⋅meanᵗ(uˡ,pˡ,νˡ))) - 
       ((n_Γ⋅meanᵗ(vˡ,qˡ,νˡ))⋅jumpᵘ(uˡ,uˢ)) )dΓ

  l((vˡ,qˡ,kˡ,vˢ,kˢ)) = 0.0
  ...
  writevtk([dΩˢ,dΓ],"res_s_$i",cellfields=["uhs"=>uhs])
  ...
  while t < T
    ...
    X,Y,dΩˡ,dΩˢ,Ωˡ,Ωˢ,dΓ,n_Γ,cₜ = update_all!(t,Δt,uhs(cₜ))
